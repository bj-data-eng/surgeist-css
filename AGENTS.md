...
Use this guide for automated work in the `surgeist-css` crate repo.
...
This repo owns the `surgeist-css` crate.

The top-level `surgeist` repo is the facade crate and integration workspace. It links this repo as a submodule under `crates/surgeist-css` and owns workspace wiring, submodule pointers, cross-crate plans, source-derived API coordination, and whole-system verification.
...
`surgeist-css` owns strict CSS parsing and lowering into typed style data.
...
1. Confirm the work belongs in `surgeist-css`.
