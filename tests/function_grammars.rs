...
fn assert_clip_path_rejected(value: &str) {
    ...
    assert_eq!(
        report.syntax().len(),
        1,
        "retained invalid clip-path `{value}`: {:?}",
        report.syntax()
    );
    assert_eq!(
        report.syntax()[0]
...
fn parsed_clip_path_property(value: &str) -> CssClipPathPropertyValue {
    ...
    let declaration = report.syntax()[0]
        .known()
        .expect("known clip-path declaration");
    let CssKnownPropertyValueRef::ClipPath(value) = declaration
        .property_value()
        .expect("ordinary clip-path value")
    else {
...
#[test]
fn circle_percentage_remains_legacy_only_without_entering_the_current_model() {
    let property = parsed_clip_path_property("circle(25% at center)");
    assert!(property.current().is_none());
    assert!(property.i01_subset().is_some());
}

#[test]
fn every_radial_extent_keyword_is_typed_for_circle_and_ellipse() {
    for (keyword, expected) in [
        ...
        let Some(CssClipPathValue::BasicShape(CssBasicShapeValue::Circle(circle))) =
            ...
            panic!("expected typed circle extent `{keyword}`");
        };
        ...
        let Some(CssClipPathValue::BasicShape(CssBasicShapeValue::Ellipse(ellipse))) =
            ...
            panic!("expected typed ellipse extent `{keyword}`");
        };
...
#[test]
fn selected_basic_shapes_expose_exact_typed_authored_components() {
    ...
    let radii = inset.round().expect("inset round radii");
...
#[test]
fn omitted_shape_radii_and_polygon_prefix_are_explicit_current_branches() {
    ...
    let Some(CssClipPathValue::BasicShape(CssBasicShapeValue::Ellipse(ellipse))) =
        ...
        panic!("expected positioned default ellipse");
    };
